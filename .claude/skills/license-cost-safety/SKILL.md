...
This project's entire output is "licences you should remove" — that makes the read-only boundary more load-bearing here than in a report-only audit, since every finding is one API call away from being actioned directly instead of going through a change ticket.

1. Treat the runtime as read-only for Microsoft 365, Exchange, and Entra ID configuration. Do not add `Set-`, `New-`, `Remove-`, `Enable-`, `Disable-`, or `Update-` mutation cmdlets to runtime code — explicitly including `Set-MgUserLicense`, `Set-MsolUserLicense`, `Set-AzureADUserLicense`, `Update-MgUser`, and `Set-Mailbox`.
...
5. Retrieve secrets only from Key Vault via the managed identity. Never place tokens, private keys, credentials, mailbox contents, subjects, licence cost figures, or client spreadsheets in source, logs, fixtures, or prompts.
6. Classify missing or unavailable evidence as `Unknown` — this applies especially to sign-in activity in tenants without Entra ID P1/P2 (see `graph-exchange-readonly`) and to licence pricing, which Graph never provides. Do not make a waste finding from absent or assumed data.
